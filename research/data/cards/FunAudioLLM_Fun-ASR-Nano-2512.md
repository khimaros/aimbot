...
> ⚡ **CPU / edge — no GPU, no Python:** run Fun-ASR-Nano as a single self-contained binary via **llama.cpp / GGUF** (like whisper.cpp), with built-in VAD. Prebuilt binaries + one-command model download → [Fun-ASR-Nano-GGUF](https://huggingface.co/FunAudioLLM/Fun-ASR-Nano-GGUF) · [runtime](https://github.com/modelscope/FunASR/tree/main/runtime/llama.cpp) · [guide](https://www.funasr.com/en/blog/funasr-llama-cpp-whisper-cpp-alternative.html)
