...
> *For running Nemotron 3.5 Lightning on Local AI devices (RTX 5090, DGX Spark, RTX 6000 Pro) with the [GGUF checkpoint](https://huggingface.co/ggml-org/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-GGUF) - please see: [NVIDIA-Nemotron-3.5-Lightning-30B-A3B-NVFP4#Local AI](https://huggingface.co/nvidia/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-NVFP4#local-ai-rtx-5090-dgx-spark-and-rtx-6000-pro)*
...
|---|---|---|---|
| [**1× H100 80GB**](#1x-h100-vllm) | vLLM (or [SGLang](#1x-h100-sglang)), BF16 | None | 256K (memory-bound) |
| [**8× H100 — long context**](#8x-h100) | vLLM, TP8 + expert parallel | None | 1M |
| [**1× GB200**](#1x-gb200) | vLLM + DSpark | DSpark (`num_speculative_tokens 5`) | 1M |
| [**1× B200**](#1x-b200) | SGLang (balanced) | None — append MTP / DFlash / DSpark | 1M |

These recipes serve BF16 directly for research, evaluation, and post-training. For production inference, the [NVFP4 release](https://huggingface.co/nvidia/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-NVFP4) is faster and covers more hardware. On a single H100, BF16 is memory-bound to ~256K — use 8× H100, GB200, or B200 for the full 1M window.
...
> The BF16 recipes below cover vLLM and SGLang. For TensorRT-LLM, W4A16 (Blackwell / Hopper / Ampere from a single checkpoint), and DGX Spark recipes, see the [NVFP4 card](https://huggingface.co/nvidia/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-NVFP4).
...
> For more indepth instructions on how to deploy through SGLang, head [here](https://docs.sglang.io/cookbook/autoregressive/NVIDIA/Nemotron3.5-Lightning)
...
The blocks above run the balanced baseline. To enable one of the [speculative decoding strategies](#speculative-decoding-strategies) described above, append the matching flags to any command (the DFlash and DSpark draft models are the NVFP4 checkpoints):
