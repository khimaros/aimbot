...
function affordable(m, all, room) {
  ...
  return fits.filter(c => !c.bpw || (c.bpw <= ceiling && c.bpw >= bottom));
}
...
//
// `text` stays first and stays the default: MODELS.md's ranking blocks are
// generated by booting this page with nothing set, so the default view IS the
// document's roster and moving it rewrites the document.
//
