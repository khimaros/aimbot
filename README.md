...
```
text models (61)      the default, and the roster MODELS.md ranks
any to text (72)      everything that emits text: the LLMs plus the transcribers
text to text (27)     vision to text (34)      these two partition `text models`
...
**`text models` is a classification, not a shape.** it is the registry's own
...
text` and `text models` share gemma 4, `any to text` contains both -- so picking
...
`text models` stays the default deliberately: MODELS.md's ranking blocks are
