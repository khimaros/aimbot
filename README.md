...
**the modality dropdown is how you get at the non-text half.** the roster is 87
models and 26 of them read or write something other than text, which two
...
text -> audio (6)     text -> image (5)        text -> embeddings (1)
...
**the runtime chips are what you have built.** a multi-select over the engines
that load these weights, on by default for llama.cpp, crispasr and
