...
image generation is scored by one board, artificial analysis' text-to-image
arena, on the same terms: an elo, matched only against rows the site itself
publishes weights for. krea 2 is on it and is deliberately unmatched, because
the site reports no weights for it.
...
`language models` stays the default deliberately: MODELS.md's ranking blocks are
generated by booting this page with nothing set, so the default view IS the
document's roster and moving it would rewrite the document. an option that
would select nothing is not offered rather than sitting there returning an
empty table.
