...
all modalities (89)
text -> text (27)     vision -> text (34)      these two partition the above
audio -> text (12)    transcription, kind: speech
text -> audio (8)     text -> image (5)        text -> embeddings (1)
diarization (3)       weights --diarize loads beside an ASR model
```

**pick several.** it is a multi-select, because `text and speech` is a question
somebody asks and answering it used to mean selecting `all modalities` and
reading past the image models. picking none is every shape. modality, runtime
and publisher are all the same control, for the same reason -- one publisher was
never the interesting question either.
