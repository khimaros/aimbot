...
research/models-md.json          MODELS.md's own argument, with its figures referenced
research/dashboard-table  MODELS.md's ranking, generated by running docs/
research/build-sections   MODELS.md's prose, from the two files above
...
**the error rates are stored inverted**, as the accuracy they leave: `(1 - wer)
* 100` and `100 - corpusErrorPct`, one facet per language. every facet on the
page is higher-is-better because the percentile pass has no notion of direction,
so a WER carried verbatim would rank the worst transcriber first -- and would do
it silently, since 4% and 8% both look like plausible numbers in a column. an
elo needs no such treatment and gets none.

image generation is scored by three boards, and one of them is not an arena.
artificial analysis and lmarena both publish an elo from people choosing between
two pictures, matched only against rows the site itself publishes weights for.
the genai showdown is fifteen prompts with the passing criteria written down and
a pass or fail against each, so it answers a question neither elo asks: flux.2
dev sits mid-table on both arenas and passes 5 of the 15. all three carry a
third of the generation weight, editing is carried at zero on the two boards
that rate it, and nothing is averaged into anything.
...
toggles could not express: `text` and `vision` held down together meant "a text
model that also takes images", and no combination of them meant audio in and
text out. one list of shapes replaces both, and the shapes are the ones
`llama-swap-groups` derives group membership from, so this page and a host name
the same things:

```
language models (61)  the default, and the roster MODELS.md ranks
any to text (72)      everything that emits text: those plus the transcribers
...
nor a superset of `any to text`: 34 of its members take images, video or audio
in, one emits embeddings and no text at all, and the twelve transcribers that DO
emit text are `kind: speech` and outside it. that is why both are offered.

one shape at a time, because they overlap rather than partition -- `audio to
text` and `language models` share gemma 4, `any to text` contains both -- so picking
two says less than picking the one that describes what you are after. the
runtime and publisher filters ARE multi-selects, where the union is the
question: `llama.cpp or crispasr` is something a reader wants.

`language models` stays the default because it is what a reader of this roster
is usually after. it used to stay for a second reason -- MODELS.md's ranking
blocks are generated by booting this page, so the default view WAS the
document's roster -- and `research/dashboard-table` pins what it ranks now,
modality and flags both. the licence filter is what finished that split: it
drops seven text models the document is still about, so one decision was doing
two jobs and now does one. an option that would select nothing is not offered
rather than sitting there returning an empty table.

**the runtime dropdown is what you have built.** the same control, over the
engines that load these weights, on by default for llama.cpp, crispasr and
stable-diffusion.cpp. it is a fact per model rather than a guess from the file
extension: two of the entries are derived -- llama.cpp from the architecture
...
its own reason. that toggle is gone rather than hidden, so there is one control
and the menu says which runtime each entry wants.

the same fact drives **how to run it**: the operations tab used to offer every
model a choice between `llama-server` and `vllm serve`, which is wrong for 28 of
89. it now spells the command for the runtimes that actually load the model --
`crispasr --backend whisper -m ...`, `sd --diffusion-model ... --vae ... --llm
...
percentiles, and the weights are yours.** the page opens on the composite
weights in `scripts/build-viewer` -- 0.30 coding, 0.30 agentic, 0.15 gbench, 0.10 arena,
...
a tooltip on the column that says what you would download.
...
first. the second one is gone: `composite()` and its weights are deleted, and
the weighting sensitivity, the arena table and the effective-intelligence table
run through the page with the rest. a test asserts the ranking and the
sensitivity table name the same leader, which is the thing that was not true.
...
the document's own argument moved the same way, into
`research/models-md.json`, and MODELS.md now holds no prose of its own. a figure
typed into markdown cannot be re-read at print time, and that is how the summary
came to say the leader scored 87.4 against a table under it reading 79.1, and
the qwen3.8 27b writeup to quote an artificial analysis index of 52.0 against
33.9 in the capture beside it. a section there names the model it is about and
references its figures -- including `{aa.lcr@owner/Repo}` for the model a
...
to already know; the footer now names every active filter and how many models it
...
the TOP-LEVEL aggregate stays reddit-only, because the composite weights
