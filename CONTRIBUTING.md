...
**Thinking is per-caller, not global.** `AIMBOT_LLM_EFFORT` sets how hard --
`low`, `medium`, `high`, `xhigh`, or `off` to gate it out everywhere -- and only
the prose steps ask for it. The triage steps stay off whatever it says, and the
reason is in `llm.py`: asked to name a base model, a thinking run invented
`Large Language Model (LLM) / AI Model Repository` where the plain one read it
off the repo id, and eight calls came back empty having reasoned past the
budget. That is extraction, where reasoning only finds ways to miss the one
...
make prose-usecase        # draft the verdicts whose figures moved
make prose-registry       # draft the registry notes
```

**The validator does not care who wrote the block.** That is the point. A draft
coming back from the endpoint went through refusals and prose typed by hand went
through none, which is the wrong way round: the checks are about whether a
sentence is true and readable. What the author changes is what a reader should
do about it, which is why `research/prose.json` names three -- `assistant`
(written in a session, with a human reading along), `local` (drafted by the
endpoint and not yet read by anybody) and `human`.

**A figure is referenced, never typed.** `research/verdicts.py` does this for a
verdict (`{swerebench.resolved}`) and `research/notes.py` for a registry note
(`{gib}`, `{bpw}`, `{gib@OTHER_RUNG}`). Both render at print time out of the
captures, so the class of error is removed rather than detected. vcruz305's
deepseek pack is why: its note said `95.75 gib` and `3 of 7 shards` while the
sizes capture beside it read 246.35 over seven.

**A comparison references both sides.** `{aa.lcr@owner/Repo}` resolves against
that model rather than the block's own, and renders without repeating the facet
name because both halves are the same facet. Half of `II 55.8 against 52.0` used
to be typed for want of this, inside a sentence that was otherwise generated,
and the typed half aged eighteen points.

**A claim names where it came from, and a quotation is checked.** `{card}` in a
note renders as the repo the claim came out of, and words in quotation marks in
front of one must actually appear in that card -- which is why the card bodies
are a committed capture under `research/data/cards/` rather than an http cache.
A paraphrase inside quotation marks is the one thing in the format that reads as
sourced and is not.

**MODELS.md is output.** Every paragraph in it comes from a generator, and a
paragraph typed straight into the document is a regression the inventory will
report. Where the paragraph is an argument rather than a restatement, the
generator is `research/models-md.json`: put the words there, give the section a
`subject:` when it is about one model, reference its figures, and leave a marker
in the document.
...
A section also reaches the ranking itself through `dash.quality`, `dash.rank`,
`dash.gib`, `dash.bpw`, `dash.measured` and `dash.of`, because a sentence about
who leads has to read the order rather than restate it. The summary said
qwen3.8 flash next led `at 87.4, against qwen3.8 27b's 76.8 and deepseek v4
flash 0731's 73.4` while the table under it read 79.1, 74.0 and a third place
that had become glm-5.3-flash.
...
## the registry is canonical, and so is one copy of the arithmetic

every FACT on the dashboard comes from `registry/` and the captures, through
`scripts/build-viewer`, into `docs/data.json`. nothing is measured in the page.

what the page used to own is the ARITHMETIC, and eight of its columns are
computed rather than stored -- rank, quality, evidence, active, fits ctx, fits
quant, kv gib, bpw -- because they are functions of what the reader set. those
live in `scripts/roster.js`: the fit, the kv cache, the retention curves, the
composite and the column definitions, with no dom and no localStorage.

three things read that module and none of them reimplements it.
`scripts/build-viewer` inlines it into `docs/index.html`, so the site is still
one file a reader can open off disk. `scripts/aimbot` requires it under node to
answer from a terminal. `research/dashboard-table` boots the built page to
